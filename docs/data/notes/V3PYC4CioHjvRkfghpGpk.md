...
By default, a home router will block all requests from the internet— yielding a setup where devices on a LAN can only interact with the internet via a request made by the device itself. Port forwarding is a mechanism to get around this, and allow a server from the internet to be able to send a request to a device on your LAN.
...
- [[Reverse Proxy|webserver.proxy.reverse]]: A reverse proxy can forward requests from the internet to specific devices on the LAN. This method adds an extra layer of security, as the devices on the LAN are not directly exposed. With a reverse proxy, we can opt to only open port 443 on the router, and then forward all requests through the reverse proxy on to any other local ports that need to be accessed.

## Alternatives
There are other ways that a device from the internet can send requests to devices on a LAN:
- [[VPN|network.vpn]]: You can set up a VPN to create a secure connection between the LAN and the device on the internet. This allows devices to communicate as if they were on the same local network, without exposing them directly to the internet.
- SSH Tunneling: Secure Shell (SSH) can be used to create a secure tunnel between a device on the LAN and a server on the internet. This tunnel can then be used to forward requests from the internet to the device securely.
	- not recommended
