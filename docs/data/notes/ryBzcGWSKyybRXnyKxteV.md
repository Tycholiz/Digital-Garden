...
Without services, there is no way for pods to communicate with each other. 
- The only communication that is available by default is between containers within a pod (via localhost)
...
- we need to use `port-forward` to redirect traffic from our local machine to a pod. Otherwise, a user that doesn’t have access to our Kubernetes cluster is not able to access this application
