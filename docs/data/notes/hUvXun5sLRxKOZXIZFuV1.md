...
- ex. if a policy allows the `GetUser` action, then a user with that policy can get user information from AWS. Furthermore, it does not matter if the resource is accessed from the AWS Management Console, the AWS CLI, or the AWS API.
