...
### Policy
A policy is a whitelist of permissions for an action. We then take that policy and attach it to an IAM identity (ie. user, user group or role)
- [list of actions](https://docs.aws.amazon.com/IAM/latest/APIReference/API_Operations.html)
  - ex. `AmazonGlacierFullAccess`, `AmazonGlacierReadOnlyAccess`, `AWSLambdaRole`, `secretsmanager:GetSecretValue`
