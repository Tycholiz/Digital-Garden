...
A good contract test for a deployed service will tell you if:
...
- If an expected integration is working
...
## How does it work?
When doing contract testing, you have to ask yourself: am I the consumer or the provider?
...
- The [[mock|testing.test-double]] service, specifying a port at which it will run.
...
You might wonder why would a consumer take in charge of contract creation?
...
- In this scenario for a successful interaction between the two parties, we follow the following steps:
	1. Producers test the consumer's requirements and produce a test response.
...
Pact Broker is an example of a testing broker, and it can be run both managed and self-hosted.
