...
## Overview
Websockets make it possible to open a two-way interactive communication session between the user's browser and a server
- using the `WebSocket` API, you can send messages to a server and receive event-driven responses without having to poll the server for a reply.
- While a WebSocket connection is functionally somewhat similar to standard Unix-style sockets, they are not related.

### Events
The client-side of the websocket connection can listen for events by using `addEventListener()`
