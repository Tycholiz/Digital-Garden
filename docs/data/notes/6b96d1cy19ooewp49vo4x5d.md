...
- when an item is deleted (in CouchDB, soft deleted), it's `_rev` value will be updated. Since it is considered as a new revision, when a document is deleted on the source, that deletion will be replicated to the target.
