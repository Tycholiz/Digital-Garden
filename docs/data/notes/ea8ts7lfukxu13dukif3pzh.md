...
- this is because code that catches the error may expect certain properties, such as [message](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error/message), to be present on the caught value. 
    - For example, web APIs typically throw DOMException instances, which inherit from Error.prototype.
...
- Of course, if no exception is thrown in the try block, the catch block is skipped. 
