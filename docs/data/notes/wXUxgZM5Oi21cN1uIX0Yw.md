...
The auth server (e.g. AWS Cognito, Auth0) exposes a JWKS endpoint (containing JSON) for each tenant, which contains the JWKs used to verify all JWTs issued by the auth server for that particular tenant.
