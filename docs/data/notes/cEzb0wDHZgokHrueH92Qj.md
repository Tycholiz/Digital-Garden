...
This is similar to the `any` type, but is safer because it's not legal to do anything with an `unknown` value until it has been verified as a valid type for whatever you're trying to do with it.
- ex. in the following example we do a check in the `if-statement` that verifies `names` as an array, therefore verifying that `.length` is a legitimate property (from this point on, Typescript actually recognizes `names` as type `any[]`)
