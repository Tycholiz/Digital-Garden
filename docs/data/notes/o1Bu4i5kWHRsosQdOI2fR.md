...
API Gateway is a fully managed service that allows us to *create*, *publish*, *maintain*, *monitor*, and *secure* APIs (read: a set of endpoints) at any scale
...
## Authorizers
### Lambda Authorizers
When a client makes a request to one of your API's endpoints, API Gateway calls your Lambda authorizer, which takes the caller's identity as input and returns an [[IAM policy|aws.terms.IAM#policy]] as output.
