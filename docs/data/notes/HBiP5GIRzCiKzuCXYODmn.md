...
The purpose of `useEffect` is to allow us use an escape hatch from React and synchronize with some external system like a local database, non-React widget, network, or the browser DOM. If there is no external system involved, then we shouldn't need a `useEffect`
