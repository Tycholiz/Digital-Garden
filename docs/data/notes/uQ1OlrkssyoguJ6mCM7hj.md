...
The DNS resolvers constitute the client half of the DNS system's [[client-server architecture|general.arch.client-server]]
- [[dns.nameserver]] are the server

Resolvers are often just functions that create name resolution queries and send them across a network to a [[nameserver|dns.nameserver]]. When the response is received, it hands it back to the program that requested it in the first place (e.g. a browser)
