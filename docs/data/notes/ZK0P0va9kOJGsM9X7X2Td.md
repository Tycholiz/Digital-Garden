...
## Overview
A Deployment represents an application running on your cluster.
- ex. it could be an HTTP server, a [[redis]] pod etc.

If we run a [[k8s.objects.pod]] directly and it fails, Kubernetes will not automatically reschedule it. This is why we do not run it manually, and instead use a Deployment object.
