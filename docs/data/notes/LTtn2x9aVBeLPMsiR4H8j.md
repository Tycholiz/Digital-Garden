...
## What is it?
A thread is the smallest unit of execution within a [[process|os.process]]. 

Threads within the same process share the same memory space and resources.

Because they share most of the process’s resources, threads are much more lightweight compared to Processes.

Threads can easily communicate by accessing shared variables within the same memory space.
