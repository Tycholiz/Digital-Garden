...
A provisioning profile contains 4 main components:

1. App ID/Identifier - Which app this profile is for (e.g., com.tycholiz.muzimemo)
2. Certificate(s) - Which developer certificates can sign the app
3. Device List - Multiple device UUIDs that can run the app
4. Entitlements - What capabilities the app can use (iCloud, push notifications, etc.)

A provisioning profile can have multiple devices associated with it, and it can include multiple developer certificates
- The profile essentially says: "These specific devices can run this specific app, signed by these specific developers, with these specific capabilities"

Distribution Profiles are used to submit an app to the App Store for distribution. After the app is reviewed by apple they sign the app with their own signature so that it can run on any device.
- Therefore, we use the profile for testing our app prior to sending it to Apple for approval.
...
- doing this will create a `mobileprovision` file in `~/Library/MobileDevice/Provisioning Profiles` for the iOS Simulator. The filename takes the form `<ProfileUUID>.mobileprovision`
