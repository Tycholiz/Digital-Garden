...
### Analogy: Trophic Levels
The OSI model can be related to the food chain. 

At trophic level 1 we have the producers (the autotrophs), such called because they produce their own energy. In other words, they turn inorganic matter into organic matter. Many different animals count themselves as part of this group, and they serve the layer above them (ie. the heterotrophs that consume autotrophs). The point to note here, is that it doesn’t matter if any one species performs the role of the autotroph. As long as it gets performed, the whole system works. Likewise for all other trophic levels in the chain.
...
- although known as the application layer, it is not the user interface of the applications themselves. Rather, layer 7 provides functionalities and services that user-facing software applications use to present data. 
	- If an application is like a house, then layer 7 is the foundation, not the house itself.
