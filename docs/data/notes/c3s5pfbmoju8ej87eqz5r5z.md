...
- this is where Javascript engines get their speed edge from.
- bytecode is about 50%-25% the size the average baseline machine code.
...
#### Example syntax
- Load (`Ld`) small integer (`Sm`) 42 (`[42]`) into the accumulator (`a`) register
```
LdaSmi [42]
```
