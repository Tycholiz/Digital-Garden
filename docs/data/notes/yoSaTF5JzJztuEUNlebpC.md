...
- A result of this is that our application logic only has to worry about `SELECT * FROM my_table` and RLS will handle the `WHERE user_id = my_user_id` part automagically.
- To put it another way: our queries should only contain the clauses requested by our interfaces and not the filters and conditions demanded by access control in a [[multi-tenant|general.arch.multitenancy]] data store.
...
If a user has been `GRANT`ed access to a given table, then they will be able to bypass any RLS restrictions. 
