...
- depending on which we use, the resulting `app` object will have access to the methods of the underlying platform
```ts
...
- fine for the most part, but if we are running in a [[serverless|deploy.serverless]] environment, this would be problematic.
...
- This is mostly done through constructor the constructor (constructor-based dependency injection)

The general pattern is that we create a [[provider|nestjs.providers]] by decorating a class with `@Injectable`, then we pass that class into another class via that class' `constructor` method.
