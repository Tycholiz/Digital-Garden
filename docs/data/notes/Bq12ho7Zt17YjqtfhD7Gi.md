...
-  allow traffic from the reverse proxy server.
...
- If we wanted to expose multiple servers on our LAN to internet but all we had was port forwarding, we'd have to open up all ports. With a reverse proxy, we can open up just one port and let the reverse proxy handle the task of forwarding on requests.
